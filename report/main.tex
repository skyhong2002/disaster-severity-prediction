...
\hline
LightGBM w/ Score Reconstructor & 0.6942 & 0.8299 \\
XGBoost w/ Score Reconstructor & 0.7150 & N/A \\
\textbf{Ensemble (LGBM+XGB) v1} & N/A & \textbf{0.8232} \\
\hline
LightGBM Direct (Domain Feat.) & 0.6770 & N/A \\
XGBoost Direct (Domain Feat.) & 0.7320 & N/A \\
\textbf{Ensemble Strategy B} & N/A & \textbf{0.8640} \\
\hline
\end{tabular}
\end{table}
...
Week 5 & 0.6795 \\
\hline
\textbf{Average MAE} & \textbf{0.6770} \\
\hline
\end{tabular}
\end{table}
...
Baseline 3 & 0.8056 & 1 \\
Team 20 & 0.8062 & 12 \\
\textbf{Team 5 (v0 Leaky)} & \textbf{0.8094} & \textbf{1} \\
\textbf{Team 5 (v1 Ensemble)} & \textbf{0.8232} & \textbf{3} \\
Team 4 & 0.8392 & 37 \\
Team 3 & 0.8534 & 15 \\
Baseline 2 & 0.8623 & 1 \\
\textbf{Team 5 (Strategy B)} & \textbf{0.8640} & \textbf{4} \\
Baseline 1 & 0.9117 & 1 \\
Team 10 & 1.1791 & 2 \\
\hline
\end{tabular}
...
\begin{table}[h]
\centering
\caption{Prediction Statistics for Final Ensemble (Strategy B)}
\label{tab:pred_stats}
\begin{tabular}{lcccc}
\hline
\textbf{Column} & \textbf{Mean} & \textbf{Std.} & \textbf{Min} & \textbf{Max} \\
\hline
\texttt{pred\_week1} & 1.058 & 0.992 & 0.000 & 5.000 \\
\texttt{pred\_week2} & 0.998 & 0.973 & 0.000 & 5.000 \\
\texttt{pred\_week3} & 0.951 & 0.948 & 0.000 & 5.000 \\
\texttt{pred\_week4} & 0.964 & 0.952 & 0.000 & 5.000 \\
\texttt{pred\_week5} & 0.971 & 0.971 & 0.000 & 5.000 \\
\hline
\end{tabular}
\end{table}
...
forecasting drought severity levels from meteorological data. By implementing a strict chronological holdout validation, we completely avoided time leakage and established a trustworthy local evaluation metric. We explored two strategies to resolve train-test feature discrepancy: a Score Reconstructor approach (v1) and a pure Direct Forecasting approach (Strategy B). Interestingly, while Strategy B achieved the best Local MAE (0.6770), it resulted in a Kaggle Public MAE of 0.8640. Conversely, the v1 Ensemble, despite relying on noisy reconstructed scores, achieved our best robust Kaggle score of 0.8232. This demonstrates that historical target sequences carry critical structural signals that trees can leverage, even when imputed with noise. Future work includes extracting longer rolling features and exploring sequence-based models like Temporal Fusion Transformer to better capture historical signals without leakage.
