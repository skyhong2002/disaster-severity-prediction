...
Table~\ref{tab:experiment_log} records the comprehensive ablation study conducted to evaluate the impact of historical feature construction and data leakage.

\begin{table}[h]
\centering
\caption{Ablation Study \& Kaggle Leaderboard Evaluation}
...
Ensemble (Leaky v0 + Clean v1) & N/A & 0.8851 \\
Strategy B (Pure Weather Direct) & 0.6770 & 0.8640 \\
Long-Term Weather (365 days) & 0.4888 & 0.8604 \\
\textbf{Ensemble v1 (Score Reconstructor)} & 0.6942 & \textbf{0.8232} \\
...
approximately 40\% of the test data. Team 5 achieved a robust public MAE of \textbf{0.8232} using the LightGBM and XGBoost ensemble, maintaining competitive performance while ensuring zero data leakage.
...
Baseline 3 & 0.8056 & 1 \\
Team 20 & 0.8062 & 12 \\
\textbf{Team 5 (v0 Magic Leak)*} & \textbf{0.8094} & \textbf{1} \\
...
Team 10 & 1.1791 & 2 \\
\hline
\multicolumn{3}{l}{\footnotesize *Discarded from final evaluation due to data leakage.}
\end{tabular}
\end{table}

The generated submission contains one row per region and five predicted scores.
Table~\ref{tab:pred_stats} summarizes the prediction distribution for our champion model (Ensemble v1). The mean
...
\begin{table}[h]
\centering
\caption{Prediction Statistics for Champion Model (Ensemble v1)}
...
\textbf{4. The Champion Model (0.8232):} Our proposed two-stage Ensemble v1 remains the optimal solution. By using a machine learning Reconstructor to dynamically estimate the missing 91-day gap based on weather, it safely bridges the gap without leakage, capturing both the underlying historical severity and current weather dynamics.

% ========================================================
\section{Conclusion}
In this work, we implemented a robust time series prediction framework for forecasting drought severity levels. By implementing a strict chronological holdout validation, we successfully eliminated time-leakage. Our extensive ablation study revealed a compelling ``Kaggle Paradox'': pure long-term meteorological models perform exceptionally well locally but fail to generalize to the leaderboard because they lack unobserved geographical and infrastructural confounders implicitly encoded in the historical drought scores. We demonstrated that naive target persistence fails completely (MAE 1.0815), while a two-stage Score Reconstructor ensemble strikes the perfect balance, achieving a highly competitive, academically rigorous Public MAE of 0.8232.
